...
namespace Mirror
{
	...
	public static class MessagePacking
	{
		...
		public static byte GetId<T>() where T : struct, NetworkMessage
		{
			// paul: 16 bits is enough to avoid collisions
			//  - keeps the message size small
			//  - in case of collisions,  Mirror will display an error
			return (byte)(typeof(T).FullName.GetStableHashCode() & 0xFFFF);
		}
		...
		public static bool Unpack(NetworkReader messageReader, out byte msgType)
		{
			// read message type
			try
			{
				msgType = messageReader.ReadByte();
				return true;
			}
