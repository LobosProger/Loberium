...
#pragma warning restore 618
		...
		internal static ushort GetNextNetworkId() => nextNetworkId++;
		...
		internal void InitializeNetworkBehaviours()
		{
			...
			NetworkBehaviours = GetComponentsInChildren<NetworkBehaviour>(); //TODO: Changed
...
#pragma warning restore 618
		...
		internal void OnStartServer()
		{
			...
			if (NetworkServer.removedNetID.Count > 0)
			{
				netId = NetworkServer.removedNetID.Pop();
			}
			else
			{
		...
		internal void HandleRemoteCall(int componentIndex, int functionHash, MirrorInvokeType invokeType, NetworkReader reader, NetworkConnectionToClient senderConnection = null)
		{
			...
			if (!RemoteCallHelper.InvokeHandlerDelegate((short)functionHash, invokeType, reader, invokeComponent, senderConnection))
			{
		...
		// Runs on server
		internal CommandInfo GetCommandInfo(int componentIndex, short cmdHash)
		{
			// check if unity object has been destroyed
			if (this == null)
			{
