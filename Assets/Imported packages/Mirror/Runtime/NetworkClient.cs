...
namespace Mirror
{
	...
	public static class NetworkClient
	{
		...
		internal static readonly Dictionary<byte, NetworkMessageDelegate> handlers =
		...
		public static Action delegatesClientMessages;
		...
		internal static void ApplySpawnPayload(NetworkIdentity identity, SpawnMessage message)
		{
			...
			identity.transform.localPosition = new Vector3(message.position.x, message.position.y, identity.transform.localPosition.z);
			identity.transform.eulerAngles = new Vector3(identity.transform.eulerAngles.x, identity.transform.eulerAngles.y, message.rotationZ);
		...
		static NetworkIdentity SpawnPrefab(SpawnMessage message)
		{
			...
			{
				GameObject obj = GameObject.Instantiate(prefab, new Vector3(message.position.x, message.position.y, prefab.transform.position.z), Quaternion.Euler(new Vector3(prefab.transform.eulerAngles.x, prefab.transform.eulerAngles.y, message.rotationZ)));
...
#pragma warning restore 618
		...
		/*static void OnTransformMessage(TransformMessage2D transformMessage2D)
		{
			if (spawned.TryGetValue(transformMessage2D.netId, out NetworkIdentity identity))
