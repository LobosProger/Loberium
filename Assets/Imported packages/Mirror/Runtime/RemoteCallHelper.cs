...
namespace Mirror.RemoteCalls
{
	...
	{
		static readonly Dictionary<short, Invoker> cmdHandlerDelegates = new Dictionary<short, Invoker>();

		internal static short GetMethodHash(Type invokeClass, string methodName)
		{
			// (invokeClass + ":" + cmdName).GetStableHashCode() would cause allocations.
			// so hash1 + hash2 is better.
			unchecked
			{
				short hash = (short)invokeClass.FullName.GetStableHashCode();
				return (short)(hash * 503 + methodName.GetStableHashCode());
			}
		}
		...
		internal static int RegisterDelegate(Type invokeClass, string cmdName, MirrorInvokeType invokerType, CmdDelegate func, bool cmdRequiresAuthority = true)
		{
			...
			short cmdHash = GetMethodHash(invokeClass, cmdName);
		...
		static bool CheckIfDeligateExists(Type invokeClass, MirrorInvokeType invokerType, CmdDelegate func, short cmdHash)
		{
		...
		internal static void RemoveDelegate(short hash)
		{
			cmdHandlerDelegates.Remove(hash);
		}

		static bool GetInvokerForHash(short cmdHash, MirrorInvokeType invokeType, out Invoker invoker)
		{
		...
		internal static bool InvokeHandlerDelegate(short cmdHash, MirrorInvokeType invokeType, NetworkReader reader, NetworkBehaviour invokingType, NetworkConnectionToClient senderConnection = null)
		{
		...
		internal static CommandInfo GetCommandInfo(short cmdHash, NetworkBehaviour invokingType)
		{
		...
		public static CmdDelegate GetDelegate(short cmdHash)
		{
