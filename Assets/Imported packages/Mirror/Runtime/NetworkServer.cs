...
namespace Mirror
{
	...
	public static class NetworkServer
	{
		...
		internal static Dictionary<byte, NetworkMessageDelegate> handlers =
			 new Dictionary<byte, NetworkMessageDelegate>();
		...
		//! Требуется для оптимизации, т.к. ushort ограничен лишь до 65356 значений
		//! Если сервер будет работать достаточно долго, то вскоре данные значения попросту закончатся, возникнут ошибки в работе :(
		//! Т.к. новому создаваемому объекту присваивается уникальный netId, который затем складывается на единицу
		//! Если игроки будут постоянно палить из своего оружия, то произойдет тотальный пипец, ведь если пули будут  иметь NetworkIdentity компонент,
		//! которому присваивается netId, то вскоре данные значения попросту закончатся :( *плак плак*
		public static readonly Stack<ushort> removedNetID = new Stack<ushort>();
		...
		public static Action delegatesServerMessages;
		...
		static void RegisterMessageHandlers()
		{
			...
			//RegisterHandler<CmdTransform>(NetworkTransformDictionary.GetClientRpcTransformMessage);
		}
			 ...
			 where T : struct, NetworkMessage
		{
			...
			using (PooledNetworkWriter writer = NetworkWriterPool.GetWriter())
			{
				...
				/*#if UNITY_EDITOR
								Debug.LogWarning("\tName: " + methodName + "\tConnections count: " + 1 + "\n\t\t" + "Amount of bytes: " + writer.Position + "\t\t" + "Total amount of bytes: " + writer.Position + "\t" + "Channel: " + (channelId == 0 ? "reliable" : "unreliable") + "\n");
				#endif*/
			}
		}
			 ...
			 where T : struct, NetworkMessage
		{
			...
			using (PooledNetworkWriter writer = NetworkWriterPool.GetWriter())
			{
				...
				//! Код, отвечающий за визуализацию графика и передачу данных!
				//TODO NetworkProfiler
				NetworkDiagnostics.OnSend(message, channelId, segment.Count, count);

				//! Код, отвечающий за полную визуализацию трафика
				/*#if UNITY_EDITOR
								Debug.LogWarning("\tName: " + methodName + "\tConnections count: " + 1 + "\n\t\t" + "Amount of bytes: " + writer.Position + "\t\t" + "Total amount of bytes: " + writer.Position + "\t" + "Channel: " + (channelId == 0 ? "reliable" : "unreliable") + "\n");
				#endif*/
			}
		...
		{
			if (MessagePacking.Unpack(reader, out byte msgType))
			{
		...
		public static void RegisterHandler<T>(Action<NetworkConnection, T> handler, bool requireAuthentication = true)
			 where T : struct, NetworkMessage
		{
			byte msgType = MessagePacking.GetId<T>();
		...
		internal static void SendSpawnMessage(NetworkIdentity identity, NetworkConnection conn)
		{
			...
			using (PooledNetworkWriter ownerWriter = NetworkWriterPool.GetWriter(), observersWriter = NetworkWriterPool.GetWriter())
			{
				...
				SpawnMessage message = new SpawnMessage
				{
					...
					rotationZ = identity.transform.eulerAngles.z,
					payload = payload
				};
				conn.Send(message);
			}
...
#pragma warning restore 618
		...
		static void DestroyObject(NetworkIdentity identity, DestroyMode mode)
		{
			...
			removedNetID.Push(identity.netId);
