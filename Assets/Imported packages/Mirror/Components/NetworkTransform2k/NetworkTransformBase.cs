...
public abstract class NetworkTransformBase : NetworkBehaviour
{
	...
	[Range(0, 1)] public float sendInterval = 0.060f;
	...
	private bool interpolateScale = false;
	...
	private int bufferTimeMultiplier = 1;
	...
	private ushort CompressFloat(float value)
	{
		return Compression.ScaleFloatToUShort(value, -1250, 1250, 0, 65535);
	}

	private float DecompressFloat(ushort value)
	{
		return Compression.ScaleUShortToFloat(value, 0, 65535, -1250, 1250);
	}

	public void OnCommandTransformMessage(NetworkConnection conn, CmdTransform transformMessage2D)
	{
		if (isServer)
		{
			Vector3 position = new Vector3(DecompressFloat(transformMessage2D.x) + 0.0001f, DecompressFloat(transformMessage2D.y) + 0.0001f, targetComponent.localPosition.z);
			Quaternion rotation = Quaternion.Euler(0, 0, DecompressFloat(transformMessage2D.rotZ));
			...
			if (clientAuthority)
			{
				NetworkServer.SendToReadyObservers<RpcTransform>(netIdentity, new RpcTransform { netId = transformMessage2D.netId, x = transformMessage2D.x, y = transformMessage2D.y, rotZ = transformMessage2D.rotZ, }, true, Channels.Unreliable);
			}
		}
	}

	public void OnClientRpcTransformMessage(RpcTransform transformMessage2D)
	{
		if (!isServer)
		{
			Vector3 position = new Vector3(DecompressFloat(transformMessage2D.x) + 0.0001f, DecompressFloat(transformMessage2D.y) + 0.0001f, targetComponent.localPosition.z);
	...
	void UpdateServer()
	{
		...
		{
			/*if (Vector2.SqrMagnitude((Vector2)targetComponent.transform.position - lastPosition) > sensivityPosition)
			{*/
			//!lastPosition = targetComponent.transform.position;
			//!lastRotationZ = targetComponent.transform.localEulerAngles.z;

			// send snapshot without timestamp.
			// receiver gets it from batch timestamp to save bandwidth.
			NTSnapshot snapshot = ConstructSnapshot();
			RpcTransform transformMessage2D = new RpcTransform
			{
				netId = netId,
				x = CompressFloat(snapshot.position.x),
				y = CompressFloat(snapshot.position.y),
				rotZ = CompressFloat(targetComponent.transform.localEulerAngles.z),
			};

			NetworkServer.SendToReadyObservers<RpcTransform>(netIdentity, transformMessage2D, false, Channels.Unreliable);
	...
	void UpdateClient()
	{
		...
		if (IsClientWithAuthority)
		{
			...
			if (NetworkTime.localTime >= lastClientSendTime + sendInterval)
			{
				// send snapshot without timestamp.
				// receiver gets it from batch timestamp to save bandwidth.
				/*if (Mathf.Abs(targetComponent.transform.localEulerAngles.z - lastRotationZ) > sensivityRotation)
				{*/
				//!lastPosition = targetComponent.transform.position;
				//!lastRotationZ = targetComponent.transform.localEulerAngles.z;

				NTSnapshot snapshot = ConstructSnapshot();
				CmdTransform transformMessage2D = new CmdTransform
				{
					netId = netId,
					x = CompressFloat(snapshot.position.x),
					y = CompressFloat(snapshot.position.y),
				...
				NetworkClient.Send<CmdTransform>(transformMessage2D, Channels.Unreliable);
